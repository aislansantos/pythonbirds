...
class Passaro(Ator):
    ...
    def calcular_posicao(self, tempo):
        ...
        if self._esta_voando():
            delt_t = tempo - self._tempo_de_lancamento
            self._calcular_posicao_vertical(delt_t)
            self._calcular_posicao_horizontal(delt_t)
    ...
    def _calcular_posicao_vertical(self, delt_t):
        y_atual = self._y_inicial
        angulo_radianos = self._angulo_de_lancamento
        y_atual += self.velocidade_escalar * delt_t * math.sin(angulo_radianos)
        y_atual -= (GRAVIDADE * (delt_t ** 2))/2
        self.y= y_atual

    def _calcular_posicao_horizontal(self, delt_t):
        x_atual = self._x_inicial
        angulo_radianos = self._angulo_de_lancamento
        x_atual += self.velocidade_escalar*delt_t*math.cos(angulo_radianos)
        self.x = x_atual
